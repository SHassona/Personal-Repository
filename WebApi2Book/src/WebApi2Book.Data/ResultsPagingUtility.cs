...
namespace WebApi2Book.Data
{
    public static class ResultsPagingUtility
    {
        ...
        public static int CalculatePageSize(int requestedValue, int maxValue)
        {
            if (requestedValue < 1)
                throw new ArgumentOutOfRangeException(nameof(requestedValue),
                    requestedValue, ValueLessThanOneErrorMessage);
            if (maxValue < 1)
        ...
        {
            if (pageNumber < 1)
                throw new ArgumentOutOfRangeException(Constants.CommonParameterNames.PageNumber,
                    pageNumber, ValueLessThanOneErrorMessage);
            if (pageSize < 1)
        ...
        {
            if (totalItemCount < 0)
                throw new ArgumentOutOfRangeException(nameof(totalItemCount), totalItemCount,
